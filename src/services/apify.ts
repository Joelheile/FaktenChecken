...
export async function fetchTikTokTranscript(
  ...
  try {
    if (!APIFY_API_TOKEN) {
      console.log(
        "Apify API token not found in environment variables. Returning mock transcript.",
      );
    ...
    console.log(
      `Fetching transcript for TikTok video: ${tiktokUrl} using fetch`,
    );
    ...
    if (!runResponse.ok) {
      ...
      throw new Error(
        `Failed to start Apify actor: ${errorData.error?.message || runResponse.statusText}`,
      );
    }
    ...
    if (!run || !run.id || !run.defaultDatasetId) {
      throw new Error(
        "Failed to get valid run details from Apify actor start response.",
      );
    }
    console.log(
      "Apify actor run initiated. Run ID:",
      run.id,
      "Dataset ID:",
    ...
    while (
      ...
      if (!statusResponse.ok) {
        ...
        console.warn(
          `Polling failed for run ID ${run.id}, attempt ${retries + 1}: ${errorData.error?.message || statusResponse.statusText}`,
        );
      } else {
        ...
        if (statusData && statusData.data && statusData.data.status) {
          ...
          console.warn(
            `Invalid status response for run ID ${run.id}, attempt ${retries + 1}`,
          );
        }
      }
      retries++;
    ...
    if (
      ...
      throw new Error(
        `Apify actor run timed out for run ID: ${run.id} after ${maxRetries * 2} seconds.`,
      );
    }

    if (runStatus !== "SUCCEEDED") {
      throw new Error(
        `Apify actor run ${run.id} did not succeed. Final status: ${runStatus}`,
      );
    }
    ...
    if (!datasetItemsResponse.ok) {
      ...
      throw new Error(
        `Failed to fetch dataset items: ${errorData.error?.message || datasetItemsResponse.statusText}`,
      );
    }
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      console.warn(
        "No items found in Apify dataset for URL:",
        tiktokUrl,
        "Dataset ID:",
        run.defaultDatasetId,
      );
    ...
    if (videoData.transcript && typeof videoData.transcript === "string") {
      ...
      videoData.subtitles &&
      Array.isArray(videoData.subtitles) &&
      ...
      console.warn(
        "No suitable transcript field found in Apify response:",
        videoData,
      );
      const videoId = extractTikTokVideoId(tiktokUrl);
