    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Du bist ein hilfreicher Assistent, der Websuchen durchführt, um aktuelle Informationen zu finden."
          },
          {
            role: "user",
        ...
        const followUpResponse = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "Du fasst Websuche-Ergebnisse zu relevanten Fakten zusammen. Sei präzise, neutral und gib nur gesicherte Informationen wieder. Nenne keine Quellen und schreibe einfach die relevanten Fakten."
              },
              {
                role: "user",
...
export async function performFactCheck(transcript: string, statement?: string): Promise<string> {
  try {
    if (!OPENAI_API_KEY) {
...
**Einfach erklärt:** Das Video mischt wahre Informationen mit falschen Dingen. Du solltest vorsichtig sein, was du davon glaubst.`;
    ...
    if ((!transcript || transcript.trim().length < 5) && (!statement || statement.trim().length < 3)) {
      console.log("Empty transcript and no statement, returning simple message");
